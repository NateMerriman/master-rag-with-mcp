...
def validate_table_structure():
    ...
    try:
        ...
        # Expected columns
        expected_columns = {
            'id', 'source_id', 'code_content', 'summary', 'programming_language',
            'complexity_score', 'embedding', 'summary_embedding', 'content_tokens', 'created_at'
        }
        ...
        # Check for indexes
        index_result = client.rpc("get_table_indexes", {"table_name": "code_examples"}).execute()
        
        expected_indexes = {
            'idx_code_examples_embedding_hnsw',
            'idx_code_examples_summary_embedding_hnsw',
        ...
        if index_result.data:
            actual_indexes = {idx['indexname'] for idx in index_result.data if idx['indexname']}
...
def test_basic_operations():
    ...
    try:
        ...
        test_code_example = CodeExample(
            ...
            summary="Simple Python function that prints a greeting",
            programming_language="python",
            complexity_score=2,
            embedding=[0.1] * 1536,  # Mock embedding
            summary_embedding=[0.2] * 1536  # Mock summary embedding
        )
        
        # Test INSERT
        print("  📝 Testing INSERT operation...")
        insert_result = client.table("code_examples").insert(test_code_example.to_dict()).execute()
        ...
        select_result = client.table("code_examples").select("*").eq("id", inserted_id).execute()
        ...
        print(f"✅ SELECT successful, retrieved record: {retrieved_record['programming_language']} code")
        
        # Test filtering by programming language
        print("  🔍 Testing language filtering...")
        filter_result = client.table("code_examples").select("*").eq("programming_language", "python").execute()
        
        if not filter_result.data:
            print("❌ Language filtering failed")
            return False
        
        print(f"✅ Language filtering successful, found {len(filter_result.data)} Python examples")
        
        # Test UPDATE
        print("  ✏️  Testing UPDATE operation...")
        update_result = client.table("code_examples").update({
            "summary": "Updated: Simple Python function that prints a greeting"
        ...
        delete_result = client.table("code_examples").delete().eq("id", inserted_id).execute()
    ...
    try:
        ...
        for i, code in enumerate(extracted_codes):
            ...
            if not (1 <= code.complexity_score <= 10):
                print(f"❌ Code block {i+1} invalid complexity score: {code.complexity_score}")
                return False
        
        print("✅ All extracted code blocks are valid")
        
        # Test conversion to CodeExample model
        ...
        for code in extracted_codes:
            code_example = CodeExample(
                ...
                embedding=[0.0] * 1536,  # Would be generated in real usage
...
def test_foreign_key_constraints():
    ...
    try:
        ...
        valid_code = CodeExample(
            ...
            code_content="print('Valid foreign key test')",
            programming_language="python",
            complexity_score=1
        )
        
        insert_result = client.table("code_examples").insert(valid_code.to_dict()).execute()
        ...
        invalid_code = CodeExample(
            ...
            code_content="print('Invalid foreign key test')",
            programming_language="python",
            complexity_score=1
        )
        
        try:
            invalid_result = client.table("code_examples").insert(invalid_code.to_dict()).execute()
            if invalid_result.data:
                print("⚠️  Invalid foreign key insert succeeded (constraint may not be enforced)")
                # Clean up
                client.table("code_examples").delete().eq("id", invalid_result.data[0]['id']).execute()
            else:
